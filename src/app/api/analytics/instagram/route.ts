...
export async function GET(request: NextRequest) {
  try {
    ...
    const analyticsData = {
      ...
      reach: insightsData.data?.find((item: any) => item.name === 'reach')?.values?.[0]?.value || 0,
      impressions: insightsData.data?.find((item: any) => item.name === 'impressions')?.values?.[0]?.value || 0,
      engagement: ((totalLikes + totalComments) / (profileData.followers_count || 1)) * 100,
      likes: totalLikes,
